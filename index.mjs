...
# Observabilidad de arranque. Los logs del worker llegan a CloudWatch por el agente
# que trae el AMI; si ese agente no levanta, la instancia queda MUDA. Pasó con una
# analysis box: viva y consumiendo SQS, pero sin log stream, así que era imposible
# saber si funcionaba. Se fuerza el arranque del agente y se deja rastro en la
# consola serie, legible con ec2:GetConsoleOutput aunque el agente esté caído.
# Todo va con "|| true": este bloque es diagnóstico y jamás debe impedir que
# arranque el worker (el script corre con "set -e").
systemctl restart amazon-cloudwatch-agent >/dev/null 2>&1 \\
  || echo "[boot] agente CloudWatch no disponible; logs solo en /var/log/worker.log" || true
{ echo "[boot] heimdall-worker modo=${mode} arrancando $(date -Is)"; } > /dev/console 2>/dev/null || true
